...
class GymEnvironment(object):
    ...
    def _state_to_info(self):
        ...
        state_dict = self._current_returns[self._brain_names[0]][0]
        ...
        if self.ob_space_type == "continuous":
            ...
            states = observation.reshape((1, self.ob_space_size))
        else:
    ...
    def step(self, action=None):
        ...
        if self._loaded and not self._global_done and self._global_done is not None:
            obs, rew, done, _ = self.env.step(action[0][0]) #action[0]
            if done:
